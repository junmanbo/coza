...
# 기간 제한
        ...
        df['bull'] = np.where((df['open'] > df['ma']) & (df['Slow_OSC'] > 0) & (df['MACD_OSC'] > 0), 1, 0)
        df['bear'] = np.where((df['open'] < df['ma']) & (df['Slow_OSC'] < 0) & (df['MACD_OSC'] < 0), 1, 0)
