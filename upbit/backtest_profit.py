...
def calStochastic(df, n=12, m=5, t=5):
    ...
    df['Slow_OSC_Slope'] = slow_osc - slow_osc.shift(1)
...
with open("AnalysisStrategies.txt", 'w') as f:
    ...
    for ticker in tickers:
        try:
            ...
            df['ror_bull'] = np.where((df['Slow_OSC_Slope'] > 0) & (df['Slow_OSC'] > 0), (df['close'] / df['open']) - fee, 1)
            df['ror_bear'] = np.where((df['Slow_OSC_Slope'] < 0) & (df['Slow_OSC'] < 0), (df['open'] / df['close']) - fee, 1)
            ...
            df.to_excel(f"/home/cocojun/Projects/coza/upbit/BackTesting/{ticker}-자동엑셀.xlsx")
            print("\n")
            f.write("\n")
        except:
            pass
