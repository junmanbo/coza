...
def calMACD(df, Fast=14, Slow=30, Signal=10):
...
# 기간 제한
        ...
        df['bull'] = np.where((df['MACD_OSC'] > 0) & (df['Slow_OSC'] > 0) & (df['Slow_OSC_Slope'] > 0) & (df['Slow_K'] < 80), 1, 0)
        df['bear'] = np.where((df['MACD_OSC'] < 0) & (df['Slow_OSC'] < 0) & (df['Slow_OSC_Slope'] < 0) & (df['Slow_K'] > 25), 1, 0)
        ...
        df['bull'] = np.where((df['MACD_OSC'] > 0) & (df['Slow_OSC'] > 0) & (df['Slow_OSC_Slope'] > 0) & (df['Slow_K'] < 85), 1, 0)
        df['bear'] = np.where((df['MACD_OSC'] < 0) & (df['Slow_OSC'] < 0) & (df['Slow_OSC_Slope'] < 0) & (df['Slow_K'] > 30), 1, 0)
        df['ror_bull'] = np.where((df['bull'] == 1), np.where((df['high'] > (df['open'] * 1.015)), 1.015 - fee, (df['close'] / df['open'])), 1)
        df['ror_bear'] = np.where((df['bear'] == 1), np.where((df['low'] < (df['open'] * 0.985)), 1.015 - fee, (df['open'] / df['close'])), 1)
        df['hpr'] = df['ror_bull'].cumprod() * df['ror_bear'].cumprod()
        df['dd2'] = (df['hpr'].cummax() - df['hpr']) / df['hpr'].cummax() * 100
        df['trading'] = np.where((df['ror_bull'] == 1) & (df['ror_bear'] == 1), 0, 1)
        df['win'] = np.where((df['ror_bull'] > 1) | (df['ror_bear'] > 1), 1, 0)
        df['total_trading'] = df['trading'].cumsum()
        df['total_win'] = df['win'].cumsum()
        df['승률2'] = df['total_win'] / df['total_trading'] * 100
        f.write(f"stoch+macd+rsi(1.5%)\nHPR: {df['hpr'][-2]}\nMDD: {df['dd2'].max()}\n승률: {df['승률2'][-2]}\n---------------------------------\n")

        if df['승률1'][-2] > df['승률2'][-2]:
            count1 += 1
        elif df['승률1'][-2] < df['승률2'][-2]:
            count2 += 1
        ...
        print("\n")
        f.write("\n")
    f.write(f"1번째 전략>>>\n승리: {count1} MDD 25 이상: {count_dd1}\n2번째 전략>>>\n승리: {count2} MDD 25 이상: {count_dd2}\n")
