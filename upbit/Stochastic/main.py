...
def save_info():
    for ticker in tickers:
        ...
        info[ticker]['slow_osc'] = calStochastic_day(df)[0]
        info[ticker]['slow_osc_slope'] = calStochastic_day(df)[1]
    ...
    try:
        ...
        if now.hour == 9 and now.minute == 0 and 30 <= now.second <= 33:
            ...
            for ticker in tickers:
                ...
                # 조건 만족시 롱 포지션
                elif total_hold < 5 and info[ticker]['position'] == 'wait' and \
            ...
            for ticker in tickers:
                ...
                high = df['high'][-1]
                # 익절한 코인 체크
                if info[ticker]['position'] == 'long' and info[ticker]['high'] > info[ticker]['price'] * profit:
                    total_hold -= 1
                    info[ticker]['position'] = 'wait'
