...
while True:
    try:
        for symbol in symbols:
            ...
            print(f"현재시간: {now} 코인: {symbol}\nStochastic K: {stochastic['slow_k'][-1]} Stochastic OSC: {stochastic['slow_osc'][-1]}\nMACD: {macd['MACD'][-1]} MACD OSC: {macd['MACD_OSC'][-1]}\n")
            ...
            # 조건을 만족하면 지정가 매수
            elif temp[symbol]['hold'] == False and total_hold < 3 and stochastic['slow_k'][-1] < 40 and stochastic['slow_osc'][-1] >= 0 and macd['MACD_OSC'][-1] < 0:
